...
def words_to_idx(words, w2i, no_BE=False, device='cpu'):
    """
    Input: [ ['I', 'am', 'hero'],
             ['You', 'are 'geneus'] ]
    output:
...
def hs_to_idx(hs_t, w2i, no_BE=False, device='cpu'):
    """ Zero-padded when word is not available (teated as <UNK>)
    ...
    w2i_hpu, l_hpu = words_to_idx(hpu_t, w2i, no_BE=no_BE, device=device)
    return w2i_hpu, l_hpu, l_hs


# Encoding ---------------------------------------------------------------------

def encode(lstm, wemb_l, l, return_hidden=False, hc0=None, last_only=False, device='cpu'):
    ...
    wenc = wenc[perm_idx_inv].to(device)
...
def encode_hpu(lstm, wemb_hpu, l_hpu, l_hs, device='cpu'):
...
def get_bert_output_s2s(model_bert, tokenizer, nlu_t, hds, sql_vocab, max_seq_length, device='cpu'):
    """
...
def get_bert_output(model_bert, tokenizer, nlu_t, hds, max_seq_length, device='cpu'):
    """
...
def get_wemb_n(i_nlu, l_n, hS, num_hidden_layers, all_encoder_layer, num_out_layers_n, device='cpu'):
    """
...
def get_wemb_h(i_hds, l_hpu, l_hs, hS, num_hidden_layers, all_encoder_layer, num_out_layers_h, device='cpu'):
    """
    As if
...
def get_wemb_bert(bert_config, model_bert, tokenizer, nlu_t, hds, max_seq_length, num_out_layers_n=1, num_out_layers_h=1, device='cpu'):

    # get contextual output of all tokens from bert
    all_encoder_layer, pooled_output, tokens, i_nlu, i_hds,\
    l_n, l_hpu, l_hs, \
    nlu_tt, t_to_tt_idx, tt_to_t_idx = get_bert_output(model_bert, tokenizer, nlu_t, hds, max_seq_length, device=device)
...
def get_wemb_h_FT_Scalar_1(i_hds, l_hs, hS, all_encoder_layer, col_pool_type='start_tok', device='cpu'):
    """
    As if
    [ [table-1-col-1-tok1, t1-c1-t2, ...],
       [t1-c2-t1, t1-c2-t2, ...].

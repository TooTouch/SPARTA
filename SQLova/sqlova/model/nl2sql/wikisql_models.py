...
class Seq2SQL_v1(nn.Module):
    def __init__(self, iS, hS, lS, dr, n_cond_ops, n_agg_ops, old=False, device='cpu'):
        ...
        self.scp = SCP(iS, hS, lS, dr, device=device)
        self.sap = SAP(iS, hS, lS, dr, n_agg_ops, old=old, device=device)
        self.wnp = WNP(iS, hS, lS, dr, device=device)
        self.wcp = WCP(iS, hS, lS, dr, device=device)
        self.wop = WOP(iS, hS, lS, dr, n_cond_ops, device=device)
        self.wvp = WVP_se(iS, hS, lS, dr, n_cond_ops, old=old, device=device) # start-end-search-discriminative model
...
class SCP(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_sc=False):
        ...
        wenc_n = encode(self.enc_n, wemb_n, l_n,
                        ...
                        hc0=None,
                        last_only=False,
                        device=self.device)  # [b, n, dim]

        wenc_hs = encode_hpu(self.enc_h, wemb_hpu, l_hpu, l_hs, device=self.device)  # [b, hs, dim]
...
class SAP(nn.Module):
    def __init__(self, iS=300, hS=100, lS=2, dr=0.3, n_agg_ops=-1, old=False, device='cpu'):
...
class WNP(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_wn=False):
        ...
        wenc_hs = encode_hpu(self.enc_h, wemb_hpu, l_hpu, l_hs, device=self.device)  # [b, mL_hs, dim]
...
class WOP(nn.Module):
    def __init__(self, iS=300, hS=100, lS=2, dr=0.3, n_cond_ops=3, device='cpu'):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, wn, wc, wenc_n=None, show_p_wo=False):
        ...
        wenc_hs_ob = wenc_hs_ob.to(self.device)
...
class WVP_se(nn.Module):
    ...
    """
    def __init__(self, iS=300, hS=100, lS=2, dr=0.3, n_cond_ops=4, old=False, device='cpu'):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, wn, wc, wo, wenc_n=None, show_p_wv=False):
        ...
        wenc_op = wenc_op.to(self.device)
...
def Loss_sw_se(s_sc, s_sa, s_wn, s_wc, s_wo, s_wv, g_sc, g_sa, g_wn, g_wc, g_wo, g_wvi, device='cpu'):
    """
    ...
    """
    loss = 0
    loss += Loss_sc(s_sc, g_sc, device=device)
    loss += Loss_sa(s_sa, g_sa, device=device)
    loss += Loss_wn(s_wn, g_wn, device=device)
    loss += Loss_wc(s_wc, g_wc, device=device)
    loss += Loss_wo(s_wo, g_wn, g_wo, device=device)
    loss += Loss_wv_se(s_wv, g_wn, g_wvi, device=device)

    return loss

def Loss_sc(s_sc, g_sc, device='cpu'):
    loss = F.cross_entropy(s_sc, torch.tensor(g_sc).to(device))
    return loss


def Loss_sa(s_sa, g_sa, device='cpu'):
    loss = F.cross_entropy(s_sa, torch.tensor(g_sa).to(device))
    return loss

def Loss_wn(s_wn, g_wn, device='cpu'):
    loss = F.cross_entropy(s_wn, torch.tensor(g_wn).to(device))

    return loss

def Loss_wc(s_wc, g_wc, device='cpu'):
...
def Loss_wo(s_wo, g_wn, g_wo, device='cpu'):
...
def Loss_wv_se(s_wv, g_wn, g_wvi, device='cpu'):
    """

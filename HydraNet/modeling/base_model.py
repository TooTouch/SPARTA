...
class BaseModel(object):
    ...
    def predict_SQL_with_EG(self, engine, dataset: SQLDataset, beam_size=5, model_outputs=None, top_k=3):
...
#             -> query = {0 : {"agg": aggs, "sel": selects, "conds": []}, 1: {"agg": aggs, "sel": selects, "conds": []}, ..}
            ...
            for i in range(len(conditions)):
                try:
                    query_k = {"agg" : aggs[i], "sel" : selects[i], "conds" : []}
                    query[i] = query_k
                except:
                    query_k = {"agg" : aggs[0], "sel" : selects[0], "conds" : []}
                    query[i] = query_k
            ...
            for idx, condition in enumerate(conditions):
                ...
                word_start, word_end = input_feature.subword_to_word[wc_topk[idx]][vs_topk[idx]], input_feature.subword_to_word[wc_topk[idx]][ve_topk[idx]]
                char_start = input_feature.word_to_char_start[word_start]
                char_end = len(input_feature.question)
                if word_end + 1 < len(input_feature.word_to_char_start):
                    char_end = input_feature.word_to_char_start[word_end + 1]
                ...
                try :
                    ...
                    if len(value_span_text_topk) < idx:
                        query[idx]["conds"] = [[int(wc_topk[idx]), int(op_topk[idx]), value_span_text_topk[0]]]
...
#         return sqls
    ...
    def beam_parse_output(self, input_feature, model_output, beam_size=5, top_k=3):
        def get_span(i):
            offset = 0
            segment_ids = np.array(input_feature.segment_ids[i])
            for j in range(len(segment_ids)):
